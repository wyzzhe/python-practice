...
规则：
1. vr_point_id：必须填写提示词开头【当前 VR 点位 ID：X】中的数字 X
2. view_direction：必须填写商品所在的图片方向（f/b/l/r），对应提示词中告知的图片顺序
3. bbox：必须填写商品在对应方向图片中的边界框（归一化坐标0-1）
4. 同一商品在多视角中只记录一次，综合多视角信息完善描述，但bbox和view_direction必须对应商品最清晰可见的那个视角
5. 多颜色/多材质如实记录
6. 未出现的信息留空字符串/空数组/null
